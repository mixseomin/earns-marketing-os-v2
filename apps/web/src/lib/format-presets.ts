...
// maxLength = đòn bẩy độ dài THẬT cho Astrolas (engine fill toward cap; directive text gần như bị bỏ qua).
// VERIFY 2026-06-16: cap≈words×6.5 cho output bám target (long maxLen2600 → 313-393 từ, 3 lần ổn định).
// Caps inflate (cũ long=5000) → thất thường (70-420 từ). KHÔNG nâng cap quá target.
export const FORMAT_PRESETS: FormatPreset[] = [
  { key: 'quote', label: '❝ Quote', words: 25, where: 'both', maxLength: 250,
    hint: '1 câu punchy đáng trích dẫn, KHÔNG giải thích thêm.',
    hintEn: 'One punchy, quotable line. No preamble, no explanation.' },
  { key: 'chat', label: '💬 Chat', words: 45, where: 'both', maxLength: 420,
  ...
  { key: 'reply', label: '↩ Reply', words: 90, where: 'reply', maxLength: 700,
    hint: '1 đoạn trả lời thẳng vào ý người ta, không lan man.',
    hintEn: 'One paragraph answering directly, no rambling.' },
  { key: 'comment', label: '🗨 Comment', words: 180, where: 'both', maxLength: 1300,
  ...
  { key: 'long', label: '📜 Bài dài', words: 380, where: 'both', maxLength: 2600,
  ...
  { key: 'blog', label: '📰 Bài blog', words: 700, where: 'post', maxLength: 4600,
...
export function resolveFormatDirective(formatKey: string | undefined, targetWords?: number): ResolvedFormat {
  ...
  const maxLength = preset?.maxLength ?? (words > 0 ? Math.max(250, Math.round(words * 6.5)) : 2000);

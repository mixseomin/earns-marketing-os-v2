    ...
    <div style={{ overflowX: 'auto', marginTop: 4 }}>
      ...
      <table style={{ borderCollapse: 'collapse', fontSize: 10.5, width: '100%' }}>
        <thead><tr style={{ color: 'var(--muted)' }}>
          {['Symbol', 'Dir', 'Lots', 'Entry', 'In px', 'Exit', 'Out px', 'SL/TP', 'Hold', 'P&L', ''].map((h) => <th key={h} style={{ textAlign: h === 'P&L' ? 'right' : 'left', padding: '2px 8px', fontWeight: 600, borderBottom: '1px solid var(--line)' }}>{h}</th>)}
        </tr></thead>
        <tbody>
          {rows.map((t, i) => {
            ...
            return (
              <tr key={i} style={{ borderBottom: '1px solid rgba(127,140,160,0.08)', opacity: t.isOpen ? 1 : 0.5, background: t.isOpen ? 'rgba(90,200,130,0.07)' : 'transparent' }}>
                ...
                <td style={{ ...cell, textAlign: 'right', color: p == null ? 'var(--muted)' : (p >= 0 ? 'var(--ok,#5ac882)' : '#ff5470') }} title={t.isOpen ? 'floating / unrealized P&L (gross; cost applied on close)' : undefined}>
                  {p == null ? '—' : (() => {
                    ...
                    return <span>{main}{sec ? <span style={{ opacity: 0.5, fontSize: 9, marginLeft: 6 }}>{sec}</span> : null}</span>;
                  })()}
                </td>

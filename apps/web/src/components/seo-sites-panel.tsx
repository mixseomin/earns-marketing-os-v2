...
const SITE_META: Record<string, { project?: string; emoji: string; review?: string }> = {
  ...
  'cities.gg': { project: 'cities-gg', emoji: '🏙️', review: '2026-10-08' },  // 09-09 review: MIXED - impr 913 (-21%), clicks 3, pos 47.0 (flat vs 47.4), 495 URLs w/ impr (+8%, still indexing). FIXED THIS PASS: apex locale roots /nl /es /fr /de /it /pt /ja /zh /ko /ru /ar all returned 404 while the page serving those queries lives at /route-planner/<locale> - /nl held 76 impr for 'routeplanner meerdere adressen' + 9 variants (pos 84-99) and had fallen from 297. next.config.ts redirects() now sends locale root -> /route-planner/<locale>, verified live (one hop, 308 -> 200). STILL OPEN: routeplanner.cities.gg/nl/ -> cities.gg/route-planner/nl/ -> 308 -> /route-planner/nl is 2 hops; the CF redirect keeps a trailing slash. Zone is NOT under the earns-assets CF token, so the rule needs the owning account. READ THE TOP QUERIES WITH CARE: the pages ranking pos 1-4 ('dr. justiniano torres aparicio...', '"fossgis_osrm_foot" "route="') are scraper queries carrying -site:/-filetype: operators, not humans - that is why 67 impr at pos 4.4 yields 0 clicks. The real human demand on this domain is the Dutch route-planner cluster.

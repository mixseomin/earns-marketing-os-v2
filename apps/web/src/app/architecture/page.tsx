...
export default async function ArchitectureRoute() {
  ...
  const [projects, busiest, caps, contentValue, contentCadence] = await Promise.all([listProjects(), busiestProjectId(), getCrewCapabilities(), getContentValue(), getContentCadence()]);
  const lite = projects.map((p) => ({ id: p.id, name: p.name }));
  const defaultProjectId = busiest && lite.some((p) => p.id === busiest) ? busiest : (lite[0]?.id || '');

  return <ArchitectureStudio projects={lite} defaultProjectId={defaultProjectId} caps={caps} contentValue={contentValue} contentCadence={contentCadence} />;
}

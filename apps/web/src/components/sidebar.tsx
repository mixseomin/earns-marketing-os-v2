...
export function Sidebar({ mode, currentProjectId, projects, currentUser, onMobileNavigate }: { mode?: Mode; currentProjectId?: string; projects: Project[]; currentUser?: CurrentUserInfo; onMobileNavigate?: () => void }) {
  ...
  const mobileTabs: MobileTab[] = pid ? [
    ...
    ...(isOperator ? [] : [
      ...
      { href: `/?p=${pid}`,       label: t('nav.phu', 'Phủ'), icon: '🛰' },
  ...
  const groups: Array<{ key: string; label: string; items: NavItem[] }> = [
    {
      ...
      items: [
        ...
        { href: `/?p=${p}`,             icon: '🛰', color: 'var(--neon-pink, #f472b6)', label: 'Phủ',          sub: 'affiliate · traffic mua · phễu', role: 'admin' },
  ...
  const groups: Array<{ key: string; label: string; items: NavItem[] }> = [
    {
      ...
      items: [
        ...
        { href: '/y-tuong', icon: '💡', color: 'var(--neon-cyan)', label: 'Ý tưởng', sub: 'sổ ý tưởng · bước · kẹt', role: 'admin' },

...
- **Bảng NHIỀU DATA mà KHÔNG tràn = `<DataTable>`** (`components/ui/data-table.tsx`, `data-comp="ui.DataTable"`). Trích từ `seo-sites-table.tsx` (reference, KHÔNG sửa): cell mono DÀY ĐẶC (`3px 5px`) + **nhóm cột bật/tắt** (`groups` + chip "Show:" → bảng rộng chỉ vẽ cái cần, persist localStorage qua `storageKey`) + **cuộn ngang gói trong container** (`overflow-x`, `margin 0 -8px`, `minWidth`) → không bao giờ tràn layout. API: `columns: DataColumn<T>[]` (mỗi cột `{key, group?, header, align?, cell(row), total?}`) + `groups: DataGroup[]` (`{key,label,color?}`). Dense là MẶC ĐỊNH (đúng mục tiêu "nhiều data"), không phải tuỳ chọn. Adopted: `affiliate-offer-table.tsx` (nhóm 7d/30d).

...
import { listOffers, listPublishers, listRegistrations, listCatalog, baseCut } from '@/lib/network/data';
...
export default async function NetworkAdminRoute({ searchParams }: {
  ...
  const [offers, publishers, registrations, report, catalog, projects, mode, cut] = await Promise.all([
    listOffers(), listPublishers(), listRegistrations(), networkReport(days || ALL_DAYS),
    listCatalog(), listProjects(), getMode('affiliate'), baseCut(),
  ]);
  return (
    <AppShell mode={mode} projects={projects} isPortfolio>
      <NetworkAdmin
        ...
        catalog={catalog} days={days} baseCut={cut}
        origin={PUB_ORIGIN}
      />
    </AppShell>
  );

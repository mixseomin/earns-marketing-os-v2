...
- **MỌI state đổi cách nhìn = nằm trong URL, F5/share giữ nguyên — không ngoại lệ (anh chốt 20/09/2026 sau khi view Tiến độ để bộ lọc trong `useState` trần).** Bộ lọc (chip/segmented/search), tab/view, nhóm, trang, dòng đang mở/expand, drawer đang mở — tất cả đọc từ URL khi mount và ghi lại khi đổi. Cách ghi: shallow `history.replaceState` trong một `useEffect` gom mọi key (mẫu: `backlinks-page.tsx` "Single source of URL truth", `tien-do-view.tsx` `tdp/tdtt/tdq/tdo`) hoặc `useUrlState().patch` (`lib/use-url-param.ts`, có RSC round-trip — dùng khi filter chạy server-side); drawer/modal = `useModalParam` (§1). Key ngắn, có tiền tố theo view để không đụng key của trang chứa (`td*`). Giá trị mặc định → xoá key (URL sạch). Viết `useState('all')` cho một bộ lọc mà không ghi URL = bug, không phải "làm sau".
- **⌘/Ctrl-click (hay chuột giữa) lên bộ lọc/view = MỞ TAB MỚI với URL đã áp lựa chọn đó** (anh chốt 20/09/2026). Đã nằm trong primitive: `Segmented`/`FilterChips` nhận `urlKey` (tên param) hoặc `hrefFor`, `ViewToggle` mặc định `view`; helper `lib/url-mo-tab.ts`. Dựng bộ lọc mới → khai `urlKey` đúng param đang ghi URL, đừng để chip nào thiếu. Chip project của Plays đã có `modOpenProjectPlays` cùng ý.

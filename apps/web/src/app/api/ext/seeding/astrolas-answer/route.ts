...
import { buildHumanizerBlock, clampDraftLength, injectTypos } from '@/lib/ai/humanizer';
...
${ctx.ai_detection_note ? `ADMIN NOTE: ${ctx.ai_detection_note}` : ''}`
  ...
  const answerClamped = injectTypos(clampDraftLength(data.answer_md, _hzOpts), _hzOpts);

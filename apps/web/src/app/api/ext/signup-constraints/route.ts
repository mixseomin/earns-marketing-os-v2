...
export async function GET(req: Request) {
  ...
  if (!key) return NextResponse.json({ ok: false, error: 'platformKey required' }, { status: 400 });
...
export async function POST(req: Request) {
  ...
  return NextResponse.json({ ok: false, error: 'field or parseError required' }, { status: 400 });
}

-- Optional images embedded in a backlink task's draft article (prep_payload.draft_images = jsonb
-- array of URLs). Rendered per format (md ![](), html <img>, plain URL) so platforms that allow
-- images can carry them. Appended at the END → CREATE OR REPLACE is safe.

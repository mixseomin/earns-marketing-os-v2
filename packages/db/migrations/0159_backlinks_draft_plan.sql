-- Surface prep_payload.draft_plan (structured seeding draft: {week, goal, items[], voice_note, ops_note,
-- ops_warn}) as a column on the backlinks view. Separate from the string `draft` (AI-writer/article flow)
-- so both coexist: draft_plan drives the per-comment review cards; draft stays the markdown fallback.
-- Additive, appended at the end → CREATE OR REPLACE is legal.

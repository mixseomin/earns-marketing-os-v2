-- ✨ Chuẩn bị điền (prepFillFields): expose prep_payload.fill_fields ({at, accountId, items:[{key,label,type,
-- value,source,confidence}]}) so the backlink drawer + ext can show/auto-fill the form values prepared for a
-- task's real captured DOM. Sibling of grounded (0144). CREATE OR REPLACE → append-only safe.

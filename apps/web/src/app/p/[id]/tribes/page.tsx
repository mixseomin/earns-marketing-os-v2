...
import { countAccountsPerTribe, listHabitatPhases, listProjectBriefTree } from '@/lib/actions/community-briefs';
...
export default async function TribesRoute({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [mode, projects, tribes, habitats, platforms, briefCounts, habitatPhases, briefTree] = await Promise.all([
    ...
    isDemo ? Promise.resolve({}) : listHabitatPhases(id),
    isDemo ? Promise.resolve([]) : listProjectBriefTree(id),
  ]);
  ...
  return (
    <AppShell mode={mode} project={project} projects={projects} tab="tribes" currentUser={me ? { id: me.id, displayName: me.displayName, email: me.email, role: me.role, specialty: me.specialty } : undefined}>
      {isDemo
        ? <TribesPage />
        : <TribesRealPage projectId={id} project={project} tribes={tribes} habitats={habitats} platforms={platforms} briefCounts={briefCounts} habitatPhases={habitatPhases} briefTree={briefTree} projectName={project.name} />}
    </AppShell>
  );
}

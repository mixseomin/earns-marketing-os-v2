-- Staff feedback loop on backlink tasks. Stored in prep_payload:
--   worker_note = free text (staff result report + opinions)
--   blocker     = { reason, at } jsonb when the staffer is stuck (absent = not blocked)
-- Appended at the END of the view → CREATE OR REPLACE is safe.

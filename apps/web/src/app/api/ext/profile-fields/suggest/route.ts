...
export async function POST(req: Request) {
  ...
  const body = await req.json().catch(() => ({})) as { identityId?: number; projectId?: string; accountId?: number; pageIntent?: string; launchName?: string; platform?: string; pinnedProjectId?: string; launchPage?: boolean; host?: string; regenerate?: boolean; focus?: string; fields?: Array<{ key?: string; label?: string; current?: string; maxLen?: number }> };
  ...
  if (db0) {
    ...
    const resolved = await resolveProjectViaTask(db0, { accountId: body.accountId, homeProjectId: pid, launchName, platform: (body.platform || '').trim(), pinnedProjectId: (body.pinnedProjectId || '').trim(), launchPage: !!body.launchPage, host: (body.host || '').trim() });

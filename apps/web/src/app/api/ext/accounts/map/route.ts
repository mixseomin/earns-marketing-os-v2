...
export async function POST(req: Request) {
  ...
  // reconcile: alias tĩnh (x→twitter) + host-slug→curated động (govloop-com→govloop) — 1 nguồn key,
  // khớp create/list/stats/readiness (tránh row-split + account mồ côi).
  const platformSlug = await reconcilePlatformKey(db, platformRaw);

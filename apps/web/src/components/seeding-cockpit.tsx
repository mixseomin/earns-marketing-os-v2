...
function isDeadStatus(s: string): boolean { return DEAD_STATUSES.includes(s); }

// Account chưa sẵn sàng để seed — subset của ACCOUNT_STATUS_META cho notReady states.
const ACCT_STATUS_META: Record<string, { label: string; color: string; dot: string; hint: string }> =
...
function notReady(s: string): boolean {
...
function expectedPlatformForKind(kind: string): string | null {
...
function platformIssue(it: SeedingQueueItem): string | null {
...
function blockingIssues(it: SeedingQueueItem): SeedingIssue[] {

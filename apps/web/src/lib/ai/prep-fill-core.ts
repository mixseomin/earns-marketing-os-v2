// Core (no auth, no 'use server') for DOM grounding + ✨ Chuẩn bị điền. Kept out of a 'use server' file
// so it is NOT exposed as an unauthenticated RPC endpoint: callers add their own auth —
// prepFillFields (server action, admin cookie) and /api/ext/tasks/[id]/prep-fill (ext token).
...
export async function getDomGrounding(db: Db, sourceUrl: string, sampleId?: number | null): Promise<{ block: string; prov: { host: string; source: string; sampleId: number | null; sampleAt: string | null } | null }> {
...
// ✨ Chuẩn bị điền core — sinh GIÁ TRỊ TỪNG FIELD (form thật) để ext auto-fill. Identity KHÔNG BAO GIỜ
// bịa: field danh tính điền deterministic từ account THẬT (thiếu → NEED:<x>); LLM chỉ lo CONTENT
// (message/subject/unknown). Password KHÔNG vào jsonb (secret-safe). Xem prepFillFields (wrapper admin).
export async function prepFillFieldsCore(db: Db, taskId: number, opts?: PrepFillOpts): Promise<{ ok: boolean; fields?: FillField[]; error?: string; grounded?: boolean; needAccount?: boolean; identity?: PrepFillIdentityMeta }> {

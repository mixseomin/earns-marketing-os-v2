...
export const USE_CASES: UseCaseSpec[] = [
  ...
  {
    slug: '8.5-ai-activity-log',
    groupKey: '8', groupLabel: 'Group 8 — Phase 8 DB wiring',
    title: 'AI Activity log /ai-log — oversight cho mọi OpenAI call',
    priority: 'high', shippedIn: 'WIP',
    featureRef: '/ai-log + listAILog action',
    tags: ['ai', 'oversight', 'cost'],
    sortOrder: 85,
    steps: [
      { n: 1, action: 'Sidebar System → click "🤖 AI Activity"', url: '/ai-log' },
      { n: 2, action: 'Verify stats strip: today cost, tokens, calls, daily budget, used %' },
      { n: 3, action: 'Filter project / model chip work' },
      { n: 4, action: 'Click row expand → suggestions output + input context JSON' },
      { n: 5, action: 'Mở Astrolas dashboard → click ↻ Refresh suggestions → quay về /ai-log → row mới hiện' },
      { n: 6, action: 'Verify cost mỗi call hợp lý ($0.0001-0.001 cho gpt-4o-mini)' },
    ],
    expected:
      'Mọi OpenAI call (skip cache) log thành 1 row. Cache hits không vào log.\n' +
      'Inspect được full prompt context + output suggestions cho debug.\n' +
      'Today cost progress bar warning khi gần budget.',
  },
  {
    slug: '8.6-per-project-ai-toggle',
    groupKey: '8', groupLabel: 'Group 8 — Phase 8 DB wiring',
    title: 'Per-project AI toggle (Settings → AI panel)',
    priority: 'medium', shippedIn: 'WIP',
    featureRef: 'ProjectSettingsForm AI panel + setProjectAIEnabled action',
    tags: ['ai', 'control', 'project'],
    sortOrder: 86,
    steps: [
      { n: 1, action: 'Astrolas Settings', url: '/p/astrolas/settings' },
      { n: 2, action: 'AI panel hiện toggle "🤖 AI bật" → click → "🚫 AI tắt"' },
      { n: 3, action: 'F5 → state persist (DB)' },
      { n: 4, action: 'Mở /p/astrolas → AISuggestionsPanel hiện error "AI tắt cho project này"' },
      { n: 5, action: 'Toggle ON lại → /p/astrolas refresh → AI suggestions chạy lại' },
    ],
    expected: 'aiEnabled flag persist DB; getOrGenerateSuggestions check trước khi call OpenAI; demos bypass flag.',
  },
  {
    slug: '8.7-daily-budget-cap',
    groupKey: '8', groupLabel: 'Group 8 — Phase 8 DB wiring',
    title: 'Daily token budget cap (env OPENAI_DAILY_BUDGET_USD)',
    priority: 'high', shippedIn: 'WIP',
    featureRef: 'getDailyTokenUsage + budget check trong getOrGenerateSuggestions',
    tags: ['ai', 'cost', 'safety'],
    sortOrder: 87,
    steps: [
      { n: 1, action: 'Verify env OPENAI_DAILY_BUDGET_USD=5 (default)' },
      { n: 2, action: 'Tạm set thấp xuống e.g. 0.001 để test' },
      { n: 3, action: 'Click ↻ Refresh AI Suggestions vài lần → khi cumulative cost vượt 0.001, error hiện "Daily budget hết"' },
      { n: 4, action: '/ai-log → today usage badge đỏ "100% used"' },
      { n: 5, action: 'Stale cache vẫn hiện (graceful degrade)' },
      { n: 6, action: 'Reset env về 5 → unblock' },
    ],
    expected: 'Hard ceiling cost. Stale cache fallback. Warning UI rõ.',
  },
  {

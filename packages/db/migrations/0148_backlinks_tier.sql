-- backlinks.tier: value tier for a backlink task (A = high-value focus, e.g. niche
-- expat/community seeding where our tool genuinely helps; B = editorial outreach;
-- C = self-serve directory). Drives the ★ badge + focus sort on the board, all
-- projects. Stored in prep_payload.tier. Appended as the LAST view column so

...
import { listOffers, listPublishers, listRegistrations, listCatalog } from '@/lib/network/data';
...
export default async function NetworkAdminRoute({ searchParams }: {
  ...
  const [offers, publishers, registrations, report, catalog, projects, mode] = await Promise.all([
    listOffers(), listPublishers(), listRegistrations(), networkReport(days || ALL_DAYS),
    listCatalog(), listProjects(), getMode('affiliate'),
  ]);
  return (

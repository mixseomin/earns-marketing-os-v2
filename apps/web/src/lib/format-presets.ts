...
export const FORMAT_PRESETS: FormatPreset[] = [
  ...
  { key: 'reply', label: '↩ Reply', words: 90, where: 'reply', maxLength: 1400,
    hint: '1 đoạn trả lời thẳng vào ý người ta, không lan man.',
    hintEn: 'One paragraph answering directly, no rambling.' },
  { key: 'comment', label: '🗨 Comment', words: 180, where: 'both', maxLength: 2400,
  ...
  { key: 'long', label: '📜 Bài dài', words: 380, where: 'both', maxLength: 5000,
  ...
  { key: 'blog', label: '📰 Bài blog', words: 700, where: 'post', maxLength: 8000,
...
export function resolveFormatDirective(formatKey: string | undefined, targetWords?: number): ResolvedFormat {
  ...
  const maxLength = preset?.maxLength ?? (words > 250 ? 4000 : words > 0 ? Math.max(800, words * 14) : 2000);
